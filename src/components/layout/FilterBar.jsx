...
const DEFAULTS = { queue: 'All Queues', segment: 'All Segments', product: 'All Products', channel: 'All Channels', dbosp: 'DB/OSP', fy: 'FY25' };
...
export default function FilterBar() {
  ...
  return (
    <div className={'filter-bar' + (showFilters ? '' : ' hidden')}>
      ...
      <select className="f-sel" value={decor.queue} onChange={(e) => setDecor((d) => ({ ...d, queue: e.target.value }))}>
        <option>All Queues</option><option>Enterprise Voice T1</option><option>Commercial Voice T2</option>
      </select>
